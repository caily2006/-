...
def build_visibility_graph(waypoints, obstacles, flight_altitude):
    """
    构建可见性图：waypoints 包含起点终点，再加上所有障碍物顶点。
    返回一个邻接表，边权重为距离（米），并确保边不与任何障碍物（考虑高度）相交。
    简化版：只考虑起点、终点和障碍物顶点，计算所有合法边。
    """
    all_points = waypoints[:]  # 起点和终点
    for obs in obstacles:
    ...
    def edge_valid(p1, p2):
        # 检查线段是否与任何障碍物（考虑高度）相交
        for obs in obstacles:
            if flight_altitude <= obs['height']:  # 仅当飞行高度不高于障碍物高度时才检测碰撞
        ...
        for j in range(i+1, n):
            if edge_valid(unique_points[i], unique_points[j]):
                dist = haversine(unique_points[i][0], unique_points[i][1],
                                 unique_points[j][0], unique_points[j][1]) * 1000  # 米
                adj[i].append((j, dist))
                adj[j].append((i, dist))
...
def auto_avoid_obstacles(start_point, end_point, obstacles, flight_altitude):
    """
    自动避让障碍物，返回航点列表（GCJ-02坐标，包含起点和终点）。
    如果直线无碰撞，返回 [start, end]；否则基于可见性图求解最短路径。
    """
    # 先检查直线是否与任何障碍物（考虑高度）相交
    straight_valid = True
    for obs in obstacles:
    ...
    if start_idx is None or end_idx is None:
        return [start_point, end_point]  # fallback
...
if st.session_state.page == "飞行监控":
    ...
    with left_col:
        ...
        auto_avoid = st.checkbox("自动避让障碍物（生成绕行航线）", value=st.session_state.auto_avoid)
        ...
        if st.session_state.a_point and st.session_state.b_point:
            ...
            if collision:
                ...
                if auto_avoid:
                    ...
                    st.success(f"✅ 已生成绕行航线，包含 {len(route)} 个航点")
                else:
    ...
    with right_col:
        if AMAP_KEY == "你的高德Key" and not use_osm:
            ...
            # 显示航线：优先显示避让后的航线，否则显示直线
            if st.session_state.a_point and st.session_state.b_point:
                if auto_avoid and collision and 'avoid_route' in st.session_state and st.session_state.avoid_route:
                    route_points = [[lat, lon] for lon, lat in st.session_state.avoid_route]
                    folium.PolyLine(route_points, color="yellow", weight=5, opacity=0.8, tooltip="避让航线").add_to(m)
                    # 标记航点
                    for i, (lon, lat) in enumerate(st.session_state.avoid_route):
                        folium.CircleMarker(
                            location=[lat, lon],
                            radius=4,
                            color='white',
                    ...
                    # 显示总距离
                    total_dist = 0
                    for i in range(len(st.session_state.avoid_route)-1):
                        total_dist += haversine(st.session_state.avoid_route[i][0], st.session_state.avoid_route[i][1],
                                                st.session_state.avoid_route[i+1][0], st.session_state.avoid_route[i+1][1])
                    folium.map.Marker(
            ...
            if show_obstacles:
                for obs in st.session_state.obstacles:
                    ...
                    popup_text = f"{obs.get('name', '障碍物')}<br>高度: {obs.get('height', 100)} m"
                    folium.Polygon(
                        locations=coords,
