...
if "map_style" not in st.session_state:          # 新增：地图样式
    st.session_state.map_style = "卫星影像"      # 可选 "卫星影像" 或 "矢量街道"
...
if st.session_state.page == "飞行监控":
    ...
    with left_col:
        ...
        st.caption("卫星影像为高德卫星图；矢量街道为二维街道地图，放大后细节更清晰。")
    ...
    with right_col:
        if AMAP_KEY == "你的高德Key":
            ...
            if st.session_state.map_style == "卫星影像":
                ...
                tiles_url = f"https://webrd01.is.autonavi.com/appmaptile?style=7&x={{x}}&y={{y}}&z={{z}}&key={AMAP_KEY}"
                attr = "高德矢量街道图"
